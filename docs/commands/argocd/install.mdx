...
- `addons`
  - `notifications` - (Optional) Continuous monitoring and notifying users of application changes.
  - `rollouts` - (Optional) Kubernetes controller and a set of CRDs that provide advanced deployment capabilities.
  - `events` - (Optional) Is an event-based dependency manager for Kubernetes.
  - `workflows` (Optional) An open source container-native workflow engine for orchestrating parallel jobs in Kubernetes.
