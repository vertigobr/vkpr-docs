...
- `addons`
  - `notifications` - (Optional) Monitoramento continuo e notifica os usuários sobre alterações na aplicação.
  - `rollouts` - (Optional) Controlador Kubernetes e um conjunto de CRDs que fornecem recursos avançados de implantação.
  - `events` - (Opcional) É um gerenciador de dependências baseado em eventos para Kubernetes.
  - `workflows` - (Opcional) Um mecanismo de fluxo de trabalho nativo de contêiner de código aberto para orquestrar trabalhos paralelos no Kubernetes.
